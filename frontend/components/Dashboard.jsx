...
function getIsolationColor(status) {
  ...
  if (status === "Well Connected")      return "text-green-600 dark:text-green-400";
  if (status === "Moderately Isolated") return "text-yellow-600 dark:text-yellow-400";
  if (status === "Highly Isolated")     return "text-red-600 dark:text-red-400";
  ...
  return (
    <div className="w-full min-h-[calc(100vh-56px)] bg-gray-50 dark:bg-gray-900 transition-colors py-10 px-4">
      <div className="max-w-2xl mx-auto flex flex-col gap-6">
        ...
        {/* Safety Mode Banner */}
        <div className={`flex items-center justify-between p-5 rounded-2xl border-l-4 shadow transition-colors ${
          safetyMode
            ? "bg-red-50 dark:bg-red-900/20 border-red-500"
            : "bg-green-50 dark:bg-green-900/20 border-green-500"
        }`}>
          <div className="flex items-center gap-3">
            <span className={`w-3 h-3 rounded-full animate-pulse ${safetyMode ? "bg-red-500" : "bg-green-500"}`} />
            <div>
              <p className={`font-semibold ${safetyMode ? "text-red-700 dark:text-red-300" : "text-green-700 dark:text-green-300"}`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm">
            <h3 className="text-blue-600 dark:text-blue-400 font-semibold mb-3">📍 Location</h3>
            {location ? (
              <div className="flex flex-col gap-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Lat</span>
                  <span className="font-mono text-gray-800 dark:text-gray-200">{location.lat.toFixed(5)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Lng</span>
                  <span className="font-mono text-gray-800 dark:text-gray-200">{location.lon.toFixed(5)}</span>
                </div>
                <hr className="border-gray-100 dark:border-gray-700" />
                <div className="flex justify-between items-center">
                  <span className="text-gray-500">Risk</span>
                  <span className={`font-bold text-sm ${getRiskColor(risk)}`}>{risk}</span>
                </div>
              </div>
            ) : (
          ...
          {/* Isolation */}
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm">
            <h3 className="text-blue-600 dark:text-blue-400 font-semibold mb-3">🔒 Isolation</h3>
            <p className={`text-lg font-bold ${getIsolationColor(isolationStatus)}`}>
              {isolationStatus}
            </p>
            {nearbyZones !== null && (
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {nearbyZones} hotspot zone{nearbyZones !== 1 ? "s" : ""} nearby
              </p>
            )}
            <p className="text-xs text-gray-400 mt-2">Area connectivity status</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            ...
            className={`py-3 rounded-xl text-white font-semibold transition ${
              safetyMode ? "bg-red-500 hover:bg-red-600" : "bg-blue-600 hover:bg-blue-700"
            }`}
          >
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
          <h3 className="text-blue-600 dark:text-blue-400 font-semibold mb-4">📞 Emergency Contacts</h3>
                ...
                {savedContacts.map((c, i) => (
                  <div key={i} className="relative group">
                    ...
                    <div className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl p-3 shadow-lg text-xs text-gray-600 dark:text-gray-300 z-10 leading-6">
                      <p><span className="font-semibold text-blue-600 dark:text-blue-400">Name:</span> {c.name}</p>
                      <p><span className="font-semibold text-blue-600 dark:text-blue-400">Phone:</span> {c.phone}</p>
                      <p><span className="font-semibold text-blue-600 dark:text-blue-400">Email:</span> {c.email}</p>
                      <p><span className="font-semibold text-blue-600 dark:text-blue-400">Relation:</span> {c.relationship}</p>
                    </div>
                  </div>
                ))}
              </div>
